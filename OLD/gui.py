...
class ExportDialog(QDialog):
    ...
    def export(self):
        ...
        if not os.path.exists(folder_name):
            msg_error = QMessageBox()
            msg_error.setIcon(QMessageBox.Warning)
            msg_error.setText("Folder does not exist!")
            msg_error.setWindowTitle("Error")
            msg_error.exec()
            return
        if not os.path.isdir(folder_name):
            msg_error = QMessageBox()
            msg_error.setIcon(QMessageBox.Warning)
            msg_error.setText("Folder is not a folder!")
            msg_error.setWindowTitle("Error")
            msg_error.exec()
            return
        if not self.flatpak_checkbox.isChecked() and not self.aur_checkbox.isChecked() and not self.official_checkbox.isChecked() and not self.pip_checkbox.isChecked():
            ...
            checked = [self.flatpak_checkbox.isChecked(), self.aur_checkbox.isChecked(), self.official_checkbox.isChecked(), self.pip_checkbox.isChecked()]
